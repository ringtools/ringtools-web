...
@Component({
  selector: 'app-dynamic-ring',
  templateUrl: './dynamic-ring.component.html',
  styleUrls: ['./dynamic-ring.component.scss']
})
export class DynamicRingComponent implements OnInit {
  ...
  getDashoffset(i: number) {
    return  117.75 + (i* (314/this.getSegments().length));
  }

  getDasharray(i: number) {
    let dash = (314/this.getSegments().length);
    let gap = 314 - dash;
    return  `${dash} ${gap}`;
  }
